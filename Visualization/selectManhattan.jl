...
function drawNetworkPDF(pb::Manhattan, name::String = "graph")
...
function drawNetworkTxt(pb::Manhattan, name::String = "graph")
  stdin, proc = open(`neato -n2 -Tplain -o Outputs/$name.txt`, "w")
  write(stdin, "digraph  citygraph {\n")
  for i in vertices(pb.network)
    write(stdin, "$i [\"pos\"=\"$(pb.positions[i].x),$(pb.positions[i].y)!\"]\n")
  end
