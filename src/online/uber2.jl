type Uber2 <: OnlineMethod
	...
	function Uber2(; removeTcall::Bool = true, removeTmaxt::Bool = true, period::Float64 = 0.0)
		offline = new()
...
"""
function onlineInitialize!(om::Uber2, pb::TaxiProblem)
	om.pb = pb
	om.pb.taxis = copy(om.pb.taxis)
	om.R = clusterCity(pb, 50)
	dateB = DateTime(2013, 03, 01, 12, 00)
	om.hCustomers = selectCustomers(length(om.pb.taxis), 0.5, dateB, 0.0, 0.0)
end
...
"""
function onlineUpdate!(om::Uber2, endTime::Float64, newCustomers::Vector{Customer}, virtualCustomers::Vector{Customer} = Customer[])
	start = om.startTime
	...
	for (i, taxiAction) in enumerate(idleTaxiActions)
		if !isempty(taxiAction.path)
			...
			newt = t + om.pb.roadTime[src(road), dst(road)]
	...
	# 		# Updates the taxi's initial location and time
	# 		om.pb.taxis[index] = Taxi(om.pb.taxis[index].id, c.orig, pickupT)
	# 	end
	# end
