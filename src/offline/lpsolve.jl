...
function lpFlow(l::FlowProblem, t::Vector{Float64}; verbose::Bool=true, solverArgs...)
    ...
    @objective(m, Max, sum(x[e]*l.profit[e] for e in edgeSet))
    ...
    sum(x[e] for e in filter( x-> x in edgeSet, in_edges(l.g, v))) == p[v])

    # all nodes : exit
    @constraint(m, cs3[v = vertices(l.g)],
    sum(x[e] for e in filter( x-> x in edgeSet, out_edges(l.g, v))) <= p[v])
