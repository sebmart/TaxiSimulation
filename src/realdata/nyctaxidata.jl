...
function loadManhattanCustomers(man::Network, fileName::AbstractString)
    ...
    FARE  = findfirst(names, "fare_amount")
    EXTRA = findfirst(names, "extra")
    TAX1  = findfirst(names, "mta_tax")
    TAX2  = findfirst(names, "improvement_surcharge")
    TOLLS = findfirst(names, "tolls_amount")

    # Constructing tree
    dataPos = Array{Float32}(undef, (2, nNodes(man)))
